...
export async function handleDeleteCommand(body) {
    ...
    if (isValidIPv4(input)) {
        ...
        if (match) {
            ...
            result = {
                success: false,
                message: 'Invalid input format. Use acc with count (e.g., bl2) or an IP address.'
            };
        }
    }
...
export async function handleQueryCommand(body) {
    ...
    try {
        ...
        const keyboard = items.map(item => {
            ...
            const vpnFilename = generateOvpnFileName(item, label).trim();
            const checkedAtDate = new Date(item.checked_at);
            const formattedCheckedAt = `${checkedAtDate.toLocaleDateString('en-GB', {
                day: '2-digit',
                month: '2-digit'
...
export async function handleFetchCommand(body) {
    ...
    if (args.length > 2) {
        if (isNaN(parseFloat(args[2]))) {
            ...
            (args.length > 3) && (country = args[3].toUpperCase());
        }
    }
    ...
    try {
        ...
        if (!foundUnusedServer) {
            if (country === 'VN') {
                ...
                await bot.message.sendMessage({
                    chat_id: chatId,
                    text: `Không tìm thấy server nào chưa sử dụng ở ở ${country}. Bạn có muốn tìm ở quốc gia khác không?\n(Đã kiểm tra ${totalProcessed} server, bỏ qua ${skipped} server đã tồn tại).`,
                    reply_markup: JSON.stringify({
